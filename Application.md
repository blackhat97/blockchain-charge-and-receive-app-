...
<td class="org-left">대전대학교 대학원</td>
<td class="org-right">711017</td>
...
물류회사와 같이 거래가 활발한 업종들은 다른 업종에 비하여 거래처의 수와 거래량이 많다(D사 기준 월 평균 대략 3천곳 이상의 거래처의 수 발생).
또한 고정거래처 뿐만 아니라 단발성 거래처도 다수 발생되어 관리 포인트가 다양하고 지역적, 위치적 상황에 따라서 동일업체의 매출이 여러 지역에서 발생된다.(지사나 지점에서 발생)
이로 인해 청구 및 수금 업무가 간단하지 않고 거래기간이 증가됨에 따라 매입누락이 발생하게 되며, 이를 곧바로 처리하지 않으면 추후 누락된 금액을 처리하기 위해 상당한 시간과 많은 인력이 소모된다.
뿐만 아니라 각 지역에 수금을 담당하는 담당자가 퇴사시 인수인계 미흡으로 인해 수금정리방법을 전수하지 않아 후임자가 오류를 일으키는 일이 빈번하고 이를 바로잡기 위해 거래처와 매입매출을 확인하는데 많은 시간과 노력이 소요된다.
하지만 많은 누적된 거래량으로 인해 정확한 매입매출의 추적이 어렵고 따라서 추적이 불가능한 매출은 미수금으로 처리되어 외부에 회사의 적정한 재무정보를 제공하지 못하게 된다.
이는 매출로 처리되지 못한 금액으로 정상적으로 처리되었을 때 보다 더 많은 세금을 부담, 납부할 가능성을 높이게 되거나, 장기간 미정리된 가수금을 불가피하게 잡이익으로 처리하여 출처를 알 수 없는 수익이 되어 회계감사상 지적으로 인한 불이익을 받을 수 있다.

이러한 문제가 발생하는 원인은 여러 기업들(판매사와 구매사들)이 서로 다른 ERP 시스템을 사용하여 기업간 거래 장부가 동기화가 되지 않고, 구매사는 자사의 편의를 위해 대금을 지급할 때 판매사가 지정한 계좌가 아닌 판매사 대표 계좌 혹은 본사 계좌(판매사의 지사에서 청구를 요청했는데도 불구하고)로 보내 판매사에서는 청구한 금액에 대한 수금 처리를 하기가 매우 힘들어진다.
또한 그렇게 보내진 대금은 구매사의 편의에 따라 여러 청구를 합쳐서 한번에 보내기도 하는데, 판매사 입장에서는 해당 금액이 어떤 거래 및 청구에서 수령한 금액인지를 재추적해야 하는 번거로움이 생긴다.
즉, 간단한 입금적요외에는 판매사의 계좌에 기록되는 정보가 매우 부족하므로 판매사는 당해 입금액이 어떠한 거래와 청구에 해당하는 것인지를 담당사원이 일일이 판매사에 문의(전화나 이메일로)해야만 하는 불편한 상황이다.
이는 역으로 구매사 입장에서도 매월마감할때마다 문의에 대응해야하므로 불편한 상황은 마찬가지이다.

따라서 Hyperledger Fabric(이하 HLF)를 이용한 블록체인 기반 청구 및 수금 솔루션을 도입해 기업간 거래시 동일한 거래 장부를 공유하고 청구 및 수금 업무를 간소화 하여 기존에 수금처리로 낭비되었던 기업들의 시간과 비용 및 잠재적인 회계문제를 줄이고자 한다.
...
따라서 기업 업종 별 특색에 따라 사용하는 ERP 시스템이 다르고 개발 및 구축비용이 막대하므로 한번 개발하면 유지관리 단계로 들어가 필요에 의하여 새로이 개발이 이뤄지는 경우가 거의 없어, 거래하는 기업간에 동일한 ERP 시스템을 사용하기란 현실적으로 극히 불가능하다.
대금지급정보가 부족함에 따라 발생한 판매사의 청구 및 수금업무의 차질은 장기화된 업무비효율로 현재는  이러한 문제를 해결하기 위해 판매사는 Vendor(벤더)사를 통해 가상계좌를 만들어 사용했지만(요즘에는 은행이 직접 가상계좌를 생성해주기도 함) 구매사는 업무관행에 익숙해 가상계좌로 보내기 보다는 본사계좌에 한번에 대금을 치르는 관행이 일상화되었다.
...
기존 시스템의 경우 경제적 위치에서 우위에 있는 구매사가 밀린 대금을 한번에 모아서 보내주고 금액에 대한 내역(일반적으로 품목, 요율, 수량)을 알려주지 않기 때문에 판매사는 어떤품목에 대한 대금인지 알기가 어렵고 수금처리를 하기 위해서 판매사는 구매사 담당직원과 전화통화를 하여 맞춰보는 등 수작업을 통해야만 해결하는 실정이었다.
...
기업에서 청구 및 미수 문제를 해결하기 위해 벤더사를 통해 가상계좌를 만드는 방식을 쓰는데, 1개의 벤더사에 월 150만원이상의 비용이 들고 있다.(비용부담은 각 사의 상황별로 다양함)
또한, 구매사의 입장에서 반드시 가상계좌를 선호하게 되는 메리트가 없기 때문에 실제로 판매사가 생성한 가상계좌로 대금을 지급해주는 빈도는 매우 낮은게 현실이다.
따라서 가상계좌가 완전한 해결책이라고 보기는 어렵다. 반면 본 시스템을 사용하게 되면 매우 적은 금액(월 10만원대 사용료)으로 해결이 가능하다


#### 정확성

기존 ERP 시스템을 사용할 경우, 판매자와 구매자 양측이 확인하는 과정이 대부분 수작업으로 이루어져 있어 오류 발생도 빈번하고 완전한 처리가 어렵지만,
본 시스템은 블록체인의 자동화된 알고리즘으로 정보를 확인하고 장부에 블록을 기록하므로 청구 및 미수금 문제를 실시간으로 정확하게 파악하므로 과오납,
미수금 등의 판매사와 구매사간 청구, 수금(지급)의 해결은 자동으로가능하다.
...
정확하게 청구 및 수금이 이뤄지지 않는 회사는 불일치로 인해 회계적으로 장기미수금 (장기화되면 불가피하게 잡이익으로도 처리하고 있음)으로 관리되어야 하는 불일치 금액의 규모가 커지게 된다.
수금이 되었는지? 안되었는지? 구매사로부터 입금액에 대한 세부정보를 정확히 수령하지 못함으로 인해 파악이 되지 않는 미수금은 회사의 내·외부적으로 문제를 일으키게 된다.

여러 지사를 거느린 회사를 예로 들면, 회사내 지사별로 수금여부를 체크할 때 어떤 지사는 통장에 입금이 되어 있지만 수금 정리를 하지 못하여 미수금이 크게 인식될 것이고 이는 당해 지사의 성과평가에서 마이너스로 평가받을 것이다.
회사의 성과평가는 팀, 사업부의 직원의성과급과 연관되므로 당해 지사의 임직원의 급여에 악영향을 주게 되는 내부적인 문제가 발생한다.
외부적으로는 회계감사인의 지적이 나오는데, 세무적으로도 미수금의 규모가 커지면, 구매사에 대한 부당거래로 보여줄 수 있다.
또한 매출채권회수가 지체되었으므로 대손충당금설정을 요구받게 되어 비용이 증가하여 회사결산상 손실처리가 과대하게 이뤄지는 문제가 발생한다.

본 시스템은 회사의 수금여부를 정확하게 파악할 수 있어 기존에 장기 미수금으로 인해 생겼던 업무의 비효율성과 회계적 이슈를 해결하고 미수금 발생 최소화 및 대손충당금으로 인한 불합리한 회계 손익처리를 방지할 수 있다.
회사입장에서 이 시스템을 사용함으로써 불필요하게 소모되는 비용을 크게 절약할 수 있고 잠재적 회계 문제를 사전에 방지하는 효과를 기대할 수 있다.

편리성, 활용성을 갖춘 모바일 앱을 기반으로 운영한다면 B2C, B2B 비즈니스에서 가상계좌를 대체할 수도 있는 파급력을 내재하였다고 결론지을 수 있다.
